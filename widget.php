...
// widget function
	class tp_widget_recent_tweets extends WP_Widget {
		...
		public function __construct() {
			...
			parent::__construct( 'haeefa_twitter_widget', '&raquo; '.HAEEFA_THEME_NAME.' Twitter' , 'haeefa', $widget_ops);
		}
			...
			public function widget($args, $instance) {
				...
				if(!empty($instance['title'])){ $title = apply_filters( 'widget_title', $instance['title'] ); }
					...
					if(!empty($tp_twitter_plugin_tweets) && is_array($tp_twitter_plugin_tweets)){
						print '<!-- twitter slide -->
							<div class="sidebarwidget" id="el-twitter-slide">
								<div class="w-twitter-content">
									<!-- twitter logo -->
									<div class="w-twitter-logo">
										<i class="fa fa-twitter"></i>
									</div>
									<!-- twitter username -->
									<div class="w-twitter-username">
										<a href=https://twitter.com/'.$instance['username'].'>'.$instance['username'].'</a>
									</div>
									<!-- twitter slide -->
							...
							foreach($tp_twitter_plugin_tweets as $tweet){					
								if(!empty($tweet['text'])){
									...
									print '<div class="item"><div class="twitter-item">'.tp_convert_links($tweet['text']).'<a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'"></a></div> <!-- twitter intents -->
											<div class="twitter-intents">
												<span>
													<a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">
													<i class="fa fa-reply"></i>Reply</a> 
												</span>
												<span>
													<a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">
													<i class="fa fa-retweet"></i>Retweet</a> 
												</span>
												<span>
													<a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">
											...
											<!-- twitter date -->
											<div class="twitter-meta-date">
												'.tp_relative_time($tweet['created_at']).'
											</div></div>';
						...
						print '</div>									
						<!-- twitter nav slide -->
									<div class="wrap-twitter-nav-slide">
										<div class="wrap-twitter-nav-slide-prev"><i class="fa fa-angle-left"></i></div>
										<div class="wrap-twitter-nav-slide-next"><i class="fa fa-angle-right"></i></div>
									</div>
								</div>
							</div>';
					}else{
		...
		//widget settings form	
			public function form($instance) {
				$defaults = array( 'consumerkey' => '', 'consumersecret' => '', 'accesstoken' => '', 'accesstokensecret' => '', 'username' => '', 'tweetstoshow' => '5' );
				...
				<p><label>' . __('Tweets to display:','tp_tweets') . '</label>
					<input type="text" name="'.$this->get_field_name( 'tweetstoshow' ).'" id="'.$this->get_field_id( 'tweetstoshow' ).'" value="'.esc_attr($instance['tweetstoshow']).'" class="small-text" /> </p>																			
